...
class TCGdexAPI {
  ...
  private cache = new Map<string, TCGdexCard | null>();

  // Memoized API call with 24-hour cache
  private fetchCardInfo = memoize(
    async (cardName: string): Promise<TCGdexCard | null> => {
      try {
        console.log(`Fetching card data from TCGdx for: ${cardName}`);
        
        // Search for card by name
        const searchUrl = `${this.baseUrl}/cards?name=${encodeURIComponent(cardName)}`;
  ...
  async getCardInfo(cardName: string): Promise<TCGdexCard | null> {
  ...
  classifyCard(cardInfo: TCGdexCard): {
    isPokemon: boolean;
    isTrainer: boolean;
    isEnergy: boolean;
    isItem: boolean;
